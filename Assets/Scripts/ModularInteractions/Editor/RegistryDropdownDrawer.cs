...
{
    /// <summary>
    /// Core property drawer for RegistryDropdownAttribute.
    /// Handles catalog-backed and animation-registry-backed types.
    /// DOOMS-specific types (Faction, InteractionPoint, Scene) fall back to a plain
    /// text field here; the DOOMS RegistryDropdownDrawer overrides those when present.
    /// Registered with useForChildren = true so derived DOOMS compat attributes are
    /// also served by this drawer when the DOOMS drawer is absent.
    /// </summary>
    [CustomPropertyDrawer(typeof(MLA_SIM.RegistryDropdownAttribute), true)]
    public class RegistryDropdownDrawer : PropertyDrawer
    {
        ...
        {
            var attrib = (MLA_SIM.RegistryDropdownAttribute)attribute;
            ...
            List<string> options = SogRegistryProvider.GetOptions(attrib.registryType).ToList();
        ...
        private static void DrawStringArray(Rect position, SerializedProperty property, GUIContent label, RegistryType registryType)
        {
